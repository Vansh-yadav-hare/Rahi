...
export const Route = createFileRoute("/ride/$rideId")({
  ...
  head: ({ loaderData }) => {
    const title = loaderData
      ? `${loaderData.from} → ${loaderData.to} · ₹${loaderData.price} | RideSure`
      : "Ride details | RideSure";
    const description = loaderData
      ? `${loaderData.date} at ${loaderData.departTime} with ${loaderData.driver.name}, trust score ${loaderData.driver.trustScore}. ${loaderData.seatsAvailable} seats left.`
      : "Verified carpool ride details on RideSure.";
    return {
      meta: [
        { title },
        ...
        <div className="space-y-6">
                  ...
                  <span className="relative flex flex-col items-center">
                    <span
                      className={`size-3 rounded-full ${
                        i === 0 || i === arr.length - 1 ? "bg-primary" : "bg-border"
                      }`}
                    />
                    {i < arr.length - 1 && (
          ...
          <div className="rounded-3xl border border-border bg-card p-7 shadow-soft">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {ride.driver.verified.map((v: string) => (
                <span
                  key={v}
