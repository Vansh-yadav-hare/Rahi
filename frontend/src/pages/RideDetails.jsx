  ...
  if (!ride) {
    return (
      <div className="mx-auto max-w-6xl px-5 py-20 text-center">
        ...
        <p className="mt-2 text-muted-foreground">This ride does not exist or has been completed.</p>
        <Link to="/search" className="mt-6 inline-block text-sm text-primary transition-smooth hover:underline">
          ← Back to search results
        </Link>
      </div>
    );
  }
  return <div className="mx-auto max-w-6xl px-5 py-10 md:py-14">
      <Link
    to="/search"
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-border bg-card p-7 shadow-soft">
            ...
            <ol className="mt-8 space-y-6">
              {[ride.from, ...ride.stops, ride.to].map((stop, i, arr) => <li key={stop} className="relative flex gap-4 pl-1">
                  <span className="relative flex flex-col items-center">
                    <span
    className={`size-3 rounded-full ${i === 0 || i === arr.length - 1 ? "bg-primary" : "bg-border"}`}
  />
                    {i < arr.length - 1 && <span className="absolute top-3 h-[calc(100%+1rem)] w-px bg-border" />}
                  </span>
                  <div className="-mt-1">
                    <div className="font-medium">{stop}</div>
                    <div className="text-xs text-muted-foreground">
                      {i === 0 ? `Departure ${ride.departTime}` : i === arr.length - 1 ? `Arrival ${ride.arriveTime}` : "Pickup point available"}
                    </div>
                  </div>
                </li>)}
            </ol>
  ...
  >
          ...
          <div className="rounded-3xl border border-border bg-card p-7 shadow-soft">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {ride.driver.verified.map((v) => <span
    key={v}
