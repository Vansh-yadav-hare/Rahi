...
export function SearchForm({ compact = false }) {
  ...
  const field = "flex flex-1 items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-3 max-md:min-h-[3.25rem] max-md:px-4 max-md:py-3.5 touch-manipulation transition-smooth hover:border-primary/40 focus-within:border-primary/60 focus-within:ring-2 focus-within:ring-ring/30 focus-within:ring-offset-2 focus-within:ring-offset-background";
  return <form
    onSubmit={onSubmit}
    className={compact ? "flex flex-col gap-4 rounded-2xl border border-border bg-card p-4 max-md:p-5 shadow-soft md:flex-row md:items-center" : "flex flex-col gap-4 rounded-3xl border border-border bg-card/95 p-4 max-md:p-5 shadow-lift backdrop-blur-xl md:flex-row md:items-center"}
  >
